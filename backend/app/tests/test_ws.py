...
def setup_bracket(client, headers, players=2):
    ...
    return tid, t["share_token"]


def test_ws_sends_snapshot_on_connect(client, auth_headers):
    tid, token = setup_bracket(client, auth_headers)
    with client.websocket_connect(f"/ws/tournaments/{tid}?token={token}") as ws:
        msg = ws.receive_json()
...
def test_ws_broadcasts_on_result(client, auth_headers):
    ...
    with client.websocket_connect(f"/ws/tournaments/{tid}?token={token}") as ws:
        ...
        match = client.get(f"/tournaments/{tid}/bracket", headers=auth_headers).json()[
            "matches"
        ][0]
        client.post(
...
def test_ws_rejects_without_token(client, auth_headers):
    tid, _ = setup_bracket(client, auth_headers)
