...
def matches(client, tid):
    return client.get(f"/tournaments/{tid}/bracket").json()["matches"]


def semis(client, tid):
    return [
        m
        for m in matches(client, tid)
...
def test_third_place_match_created(client, auth_headers):
    ...
    tp = [m for m in matches(client, t["id"]) if m["bracket"] == "THIRD_PLACE"]
    assert len(tp) == 1


def test_no_third_place_when_disabled(client, auth_headers):
    t = make(client, auth_headers, players=4, third=False)
    assert not any(m["bracket"] == "THIRD_PLACE" for m in matches(client, t["id"]))
...
def test_semifinal_losers_feed_third_place(client, auth_headers):
    t = make(client, auth_headers, players=4)
    for m in semis(client, t["id"]):
        report(client, auth_headers, m)
    tp = next(m for m in matches(client, t["id"]) if m["bracket"] == "THIRD_PLACE")
...
def test_third_place_does_not_complete_tournament(client, auth_headers):
    ...
    assert client.get(f"/tournaments/{t['id']}").json()["status"] == "ONGOING"

    final = next(
        m
        for m in matches(client, t["id"])
        if m["bracket"] is None and m["next_match_id"] is None
    )
    report(client, auth_headers, final)
    assert client.get(f"/tournaments/{t['id']}").json()["status"] == "COMPLETED"
